...
5. **使用 token 替代 cookie**，  token 在客户端也有一份，通常是存储在浏览器的本地存储（如 localStorage 或 sessionStorage）中。但是与 cookie 不同，token 不会自动发送给服务器，而是需要手动添加到请求头或请求参数中。这样，攻击者就无法利用跨站点请求来获取 token。
